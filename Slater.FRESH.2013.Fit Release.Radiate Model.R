...
fitContinuous_paleo <- function (phy, data, data.names = NULL, model = c("BM", "OU", 
                                                                         "lambda", "kappa", "delta", "EB", "white", "trend","timeshift", "release", "releaseradiate"), shift.time = 65.5, bounds = NULL, 
                                 meserr = NULL) 
{
  ...
  bounds.default <- matrix(c(1e-08, 20, 1e-07, 1, 1e-06, 1, 
                             1e-05, 2.999999, 1e-07, 50, -3, 0, 1e-10, 100, -100, 
...
fitContinuousModel_paleo <- function (ds, print = TRUE) 
{
  ...
  if (model == "BM") {
    ...
    k <- 3;
    cat("shift point is at ", shift.time, "years before present", "\n")
    start = c(log(0.1), 1)
    ...
    k <- 4;
    cat("release point is at ", shift.time, "years before present", "\n")
