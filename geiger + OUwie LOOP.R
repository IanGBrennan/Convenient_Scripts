...
write.csv(tree.model.comparison, 
          file="/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Meliphagides.Model.Comparison.BEST.csv",
          quote=F)
write.csv(total.results,
          file="/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Meliphagides.Model.Comparison.FINALTOTAL.csv",
          quote=F)
write.csv(all.shift.model.estimates,
          file="/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Meliphagides.Shift.Model.FINALPARAM.ESTIMATES.csv",
          quote=F)
...
total.results <- total.results[!(total.results$model=="JN_OLD"),]
total.results <- total.results[!(total.results$model=="NIG_OLD"),]
...
###################################################
output.total <- read.csv("/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Model.Comparison TOTAL/ALLFINAL.ALLRADIATIONS.Model.Comparison.TOTAL.csv", header=T)
...
output.total <- output.total[,c(1:6,9)] # drop the current 'w' and 'delta' estimates, we're gonna drop models
output.total <- output.total[!(output.total$model=="TS"),] # remove the TS model
output.total <- output.total[!(output.total$model=="OUS"),] # remove the OUS model
output.total <- output.total[!(output.total$model=="lambda"),] # remove the lambda model
output.total <- output.total[!(output.total$model=="kappa"),] # remove the kappa model
...
group.outz$model <- factor(group.outz$model, levels=c("BM", "OU","delta","DensDep", "EB", "fit_env", "fit_bgb", "JN", "NIG","BMS","OUMA", "OUMV", "OUMVA", "SRC", "TRC")) # this re-orders the models in the legend
...
myplot <- (ggplot(group.outz)
  ...
  + scale_fill_manual( values=wes_palette("Zissou", 15, "continuous")))

# if you just want to do a single plot
myplot <- (ggplot(outz)
  + geom_bar(aes(1, y=w, fill=outz$model), stat="identity")
  + scale_fill_manual( values=wes_palette("Zissou", 14, "continuous")))
...
####################################################
est.total <- read.csv("/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Meliphagides.Shift.Model.FINALPARAM.ESTIMATES.csv", header=T)
...
####################################################
best.models <- read.csv("/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Meliphagides.Model.Comparison.BEST.csv", header=T)
...
write.csv(best.parameter.estimates,
          file="/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Meliphagides.BEST.Model.PARAM.ESTIMATES.csv",
          quote=F, row.names = F)
...
####################################################
input <- read.csv("/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Meliphagides.BEST.Model.PARAM.ESTIMATES.csv", header=T)
input <- read.csv("/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Parameter Estimates/Australian.Marsupials.BEST.Model.PARAM.ESTIMATES.csv", header=T)
...
#sigma.sq.estimates <- subset(input, input$parameter == "sigma.sq")
#alpha.estimates    <- subset(input, input$parameter == "alpha")
#BMS.theta <- subset(BMS.estimates, rownames(BMS.estimates) == "theta")

#theta.est <- subset(est.total, est.total$parameter == "theta")
...
#######################################################
## Read in your data
model.res <- read.csv("/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/UPDATED.All.Radiations.Model.Comparison.csv")
model.res <- read.csv("/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/Meliphagides.Model.Comparison.BEST.csv")
...
########################################################################
## Read in your data
model.mult.res <- read.csv("/Users/Ian/Google.Drive/ANU Herp Work/Adaptive Radiation/Body Size Model LOOP/UPDATED.All.Radiations.Model.Comparison.csv", header=T)
