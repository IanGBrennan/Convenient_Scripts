...
#log_data1 <- read.table("bearsDivtime.1.log",header=T)
#log_data2 <- read.table("bearsDivtime.2.log",header=T)
nMCMC <- length(log_data1$originFBD)-1
burnin <- 0.2
id1 <- as.integer(nMCMC*burnin)+1
id2 <- nMCMC+1
comb_origin_data <- c(log_data1$originFBD[id1:id2],log_data1$originFBD[id1:id2])
