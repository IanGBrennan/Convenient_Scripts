setGeneric(
  name="simulateTippies",
  def=function(object="PhenotypicModel", params="numeric", method="numeric", v="logical"){standardGeneric("simulateTippies")}
)
  ...
  definition=function(object, params, method=3, v=TRUE){
    ...
    if( method == 1 ){
      ...
      if( method==2 ){
        ...
        colorChoice <- colorRampPalette(brewer.pal(9, "YlGnBu"))(length(object@tipLabels))
        #colorChoice <- rev(colorRampPalette(brewer.pal(11, "Spectral"))(length(object@tipLabels)))
        ...
        for(k in 1:5){
          findLim <- c(findLim, simulateTippies(object, params, method=1, v=FALSE) )
        }
